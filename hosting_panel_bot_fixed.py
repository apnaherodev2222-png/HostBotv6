...
"""
# ═══════════════════════════════════════════════════════════════
# FOR FUTURE AI / MAINTAINER — MASTER AUDIT PROMPT
# ═══════════════════════════════════════════════════════════════
# When auditing this file, check ALL of the following:
#
# 1. Every callback_data emitted by a button has a matching
#    `if action==` / `elif action==` branch in cb_handler().
# 2. Every branch of cb_handler() has at least one reachable
#    callback_data that triggers it (no orphan handlers).
# 3. Every user-facing screen renders consistently:
#    - Home (cmd_start + menu callback): rich card with VIDEO
#    - Bot detail, My Bots, Logs, Limits, Env: rich card via
#      _rich_edit_with_fallback / _rich_replace_or_send
#    - Failure fallback: box UI (never bare HTML if avoidable)
# 4. Every multi-step flow edits the SAME message instead of
#    sending new messages (GitHub URL input, env input, limits
#    input, admin setting input).
# 5. Auto-install runs before every pm2 start, both python and
#    node branches, in HostedBot.start() and start_pm2().
# 6. No `os.environ.copy()` is passed to a user bot — only
#    BOT_SAFE_INHERITED_ENV + that bot's decrypted vars.
# 7. No synchronous blocking call (subprocess, sqlite3, requests)
#    runs directly on the event loop — wrap with asyncio.to_thread.
# 8. All DB writes use PRAGMA busy_timeout + short transactions.
# 9. Every log_err() / log() call site passes useful context.
# 10. py_compile passes for both files.
#
# Report issues in this table format:
# | # | File | Line | Sev | Issue | Proposed fix |
# Apply patches for all HIGH severity issues.
# ═══════════════════════════════════════════════════════════════
...
# MIGRATED: single RichClient instance for this bot process, built once at
# import time. Guarded on BOT_TOKEN being set so importing this module (e.g.
# for tests/tooling) without a real token doesn't crash at import time —
# main() still prints "No BOT_TOKEN" and exits cleanly as before if missing.
rich_client = rm.RichClient(token=BOT_TOKEN) if BOT_TOKEN else None
            ...
            if self.bot_type in ("nodejs", "whatsapp"):
                ...
                if os.path.exists(pkg):
                    try:
                        ...
                        pass
                if use_npm:
                    _ok, _out = _auto_install_node_deps(self.bot_dir)
                    if not _ok:
                        log_err(f"auto-install node deps for {self.bot_id}: {_out[:400]}")
                    r = subprocess.run(
                ...
                target = os.path.join(self.bot_dir, main_file)
                _ok, _out = _auto_install_python_deps(self.bot_dir)
                if not _ok:
                    log_err(f"auto-install python deps for {self.bot_id}: {_out[:400]}")
                else:
                    log_err(f"auto-install python deps OK for {self.bot_id}: {_out[:120]}")
                r = subprocess.run(
                    ["pm2", "start", target, "--name", self.service_name] + self._resource_args()[0] + [
                     "--interpreter", sys.executable, "--cwd", self.bot_dir],
...
def _auto_install_python_deps(bot_dir: str) -> tuple[bool, str]:
    """Run pip install -r requirements.txt inside bot_dir.
    Returns (ok, output_snippet). Skips if file missing/empty."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "pip", "install",
             ...
             "-r", req],
            capture_output=True, text=True, timeout=300, cwd=bot_dir,
        )
        return r.returncode == 0, ((r.stdout or "") + (r.stderr or ""))[-1500:]
    except subprocess.TimeoutExpired:
        return False, "pip install timed out after 300s"
    except Exception as e:
        return False, f"pip failed: {type(e).__name__}: {e}"


def _auto_install_node_deps(bot_dir: str) -> tuple[bool, str]:
    """Run npm install inside bot_dir. Returns (ok, output_snippet)."""
    ...
    try:
        ...
        return False, f"npm failed: {type(e).__name__}: {e}"
...
def _clear_all_transient_states(ctx):
    """Clear every per-user transient input state before a new workflow."""
    _env_state_clear(ctx)
    limit_input_clear(ctx)
    _github_state_clear(ctx)
    ctx.user_data.pop("awaiting_zip", None)
...
async def show_env_menu(update, ctx, bid, page=0):
    ...
    try:
        ...
        ctx.user_data[f"env_page:{bid}"] = page
        ...
        blocks=[rm.heading(f"🔐 Environment: {info['name']}"),
                rm.paragraph(f"Variables: {total}/{ENV_MAX_KEYS}")]
        if chunk:
            blocks.append(rm.compact_table(["Key","Value"], [[row["env_key"], (mask_env_value(decrypt_env_value(row["encrypted_value"])) if row["is_secret"] else decrypt_env_value(row["encrypted_value"]))] for row in chunk]))
        else:
            blocks.append(rm.paragraph("No environment variables."))
        for idx,row in enumerate(chunk,start=page*ENV_PAGE_SIZE):
            blocks.append(rm.button_row([
                _rich_callback_button(f"✏️ {row['env_key'][:24]}",f"env_edit:{bid}:{idx}","primary"),
                _rich_callback_button("🗑️",f"env_del:{bid}:{idx}","danger")]))
        nav_rows=[]
        if page>0: nav_rows.append(_rich_callback_button("⬅️ Prev",f"env_list:{bid}:{page-1}","primary"))
        nav_rows.append(_rich_callback_button(f"{page+1}/{pages}",f"env_list:{bid}:{page}","primary"))
        if page<pages-1: nav_rows.append(_rich_callback_button("➡️ Next",f"env_list:{bid}:{page+1}","primary"))
        if nav_rows: blocks.append(rm.button_row(nav_rows))
        blocks.append(rm.button_row([_rich_callback_button("➕ Add New",f"env_add:{bid}","success"),_rich_callback_button("🔄 Refresh",f"env_list:{bid}:{page}","primary")]))
        blocks.append(_nav_back_row(f"env_back:{bid}"))
        msg=q.message
        if msg is not None:
            try:
                await _rich_edit_with_fallback(msg.chat_id,msg.message_id,blocks,
                    is_media=(getattr(msg,"video",None) is not None or getattr(msg,"photo",None) is not None))
                return
            except rm.RichMessageError:
                log_err(f"RICH: show_env_menu rich edit failed {bid}",traceback.format_exc())
        await safe_edit(q, "\n".join(lines), InlineKeyboardMarkup(buttons), parse_mode="HTML")
    except Exception as e:
...
async def env_begin_input(update, ctx, bid, key=None):
    ...
    if msg_src is None:
        ...
        return
    _clear_all_transient_states(ctx)
...
async def show_admin_panel(update,ctx):
    ...
    await safe_edit(q, txt, admin_main_markup(), parse_mode="HTML")
...
# MIGRATED: now backed by richmsgv2's RichClient (built-in retry, circuit
# breaker, and block validation) instead of the old bare _call_api.
# RichClient's own methods are already `async def` — they do their own
# asyncio.to_thread internally for the blocking HTTP call — so they're
# awaited directly here, NOT wrapped in another asyncio.to_thread (that
# would just produce an un-awaited coroutine and silently send nothing).
# `token` is kept for signature compatibility with existing call sites, none
# of which currently pass a non-default value.
async def _rich_send_with_fallback(chat_id: int, blocks: list, token=None) -> dict:
    client = rich_client if not token or token == BOT_TOKEN else rm.RichClient(token=token)
    if client is None:
        raise rm.RichMessageError("BOT_TOKEN is not configured")
    return await client.send(chat_id, blocks)


async def _rich_replace_or_send(chat_id: int, message_id: int, blocks: list, token=None) -> dict:
    """Rich edit -> on failure send new rich card + delete old message.
    Never falls back to plain text."""
    if not chat_id or not message_id:
        log_err("RICH: _rich_replace_or_send missing ids")
        raise rm.RichMessageError("missing chat_id/message_id")
    client = rich_client if not token or token == BOT_TOKEN else rm.RichClient(token=token)
    if client is None:
        raise rm.RichMessageError("client not configured")
    try:
        async with client._get_semaphore():
            response = await asyncio.to_thread(
                client._call_api, "editMessageText",
                {"chat_id": chat_id, "message_id": message_id,
                 "rich_message": {"blocks": blocks}},
            )
        return response
    except Exception as exc:
        log_err(f"RICH: rich edit failed, escalating: {str(exc)[:200]}")
    new_response = await client.send(chat_id, blocks)
    try:
        async with client._get_semaphore():
            await asyncio.to_thread(
                client._call_api, "deleteMessage",
                {"chat_id": chat_id, "message_id": message_id},
            )
    except Exception:
        pass
...
async def _rich_edit_with_fallback(chat_id: int, message_id: int, blocks: list, token=None, is_media: bool = False) -> dict:
    ...
    if client is None:
        raise rm.RichMessageError("BOT_TOKEN is not configured")
    # MIGRATED: RichClient.replace() already does rich-edit -> plain-edit ->
    # delete+resend internally, trying cheaper stages first — an improvement
    # over FIX_RICH_27's always-delete-first approach, which existed only
    # because the old bare edit call had no working fallback stages of its own.
    # is_media is retained for signature compatibility with existing call sites.
    return await _rich_replace_or_send(chat_id, message_id, blocks, token=token)

def err_info(text: str) -> str:
    return f"ℹ️ {text}"

def err_action(text: str) -> str:
    return f"⚠️ {text}"

def err_system(text: str) -> str:
    return f"❌ {text}"


def _nav_back_row(callback: str = "menu"):
    return rm.button_row([
        _rich_callback_button("🔙 Back", callback, "primary")
    ])

def _nav_refresh_row(callback: str):
    return rm.button_row([
        _rich_callback_button("🔄 Refresh", callback, "primary")
    ])

def _action_cancel_row(callback: str = "menu"):
    return rm.button_row([
        _rich_callback_button("❌ Cancel", callback, "danger")
    ])
...
async def _build_home_blocks(u) -> list:
    bots = await DB.get_user_bots(u.id)
    total_bots = len(bots) if u.id in ADMIN_IDS else len(
        [b for b in bots.values() if b.get("user_id") == u.id])
    running_count = 0
    for bid, info in bots.items():
        ...
        try:
            if await asyncio.to_thread(
                    HostedBot(bid, info["name"], info["dir"],
                              info["type"], info["user_id"]).is_running):
                running_count += 1
        except Exception:
            pass
    ...
    return [
        ...
        rm.compact_table(["Information", "Details"], [
            ["Owner", "@OG_SAGAR_ddos"], ["Version", "3.0"],
            ["Your Bots", str(total_bots)], ["Running", str(running_count)],
...
async def show_home_card(update, ctx):
    q = update.callback_query
    u = update.effective_user
    if not u:
        return
    blocks = await _build_home_blocks(u)
    msg = q.message
    if msg is None:
        log_err("RICH: show_home_card missing q.message")
        return
    try:
        await _rich_replace_or_send(msg.chat_id, msg.message_id, blocks)
        return
    except rm.RichMessageError:
        log_err("RICH: show_home_card rich failed, html fallback")
    await safe_edit(q,
        f"{UI.BOTS} <b>VPS Bot Manager</b>\n{UI.DIV}\n"
        f"Choose an option below 👇{credit_footer()}",
        kb_main(), parse_mode="HTML")
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await _rich_send_with_fallback(update.effective_chat.id, await _build_home_blocks(u))
        return
    except rm.RichMessageError:
...
async def cmd_pending(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        txt = (
            ...
            f"🤖 Bot: <code>{_html_text(r['name'])}</code>\n"
            f"⚙️ Type: <code>{_html_text(r['bot_type'])}</code>\n"
...
async def admin_callback_error_handler(update,ctx):
    try:
        ...
        if u and admin_only(u.id):
            ...
            if update.callback_query:
                try: await update.callback_query.answer(err_system("Admin action failed. Check logs."),show_alert=True)
                except Exception: pass
            elif update.message:
                try: await update.message.reply_text(err_system("Admin action failed. Check logs."))
                except Exception: pass
    except Exception: pass
...
async def cb_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data=="admin_broadcast" and admin_only(u.id):
        # FIX_H3: Clear *every* transient state so a stale awaiting_github_url
        # cannot swallow the broadcast text in handle_limit_or_broadcast_or_env.
        _clear_all_transient_states(ctx)
    ...
    if action=="admin_delall" and admin_only(u.id):
        ...
        for r in rows:
            ...
            if Path(r["dir"]).exists(): await asyncio.to_thread(shutil.rmtree, r["dir"], ignore_errors=True)
    ...
    if action=="admin_restart_panel" and admin_only(u.id):
        ...
        await asyncio.to_thread(subprocess.Popen,["bash","-lc",cmd],start_new_session=True); return
...
# Feature 1 callbacks remain unchanged below.
    if action=="env_menu" and bid: await show_env_menu(update,ctx,bid,0)
    ...
    elif action=="env_back" and bid:
        last_page = ctx.user_data.get(f"env_page:{bid}", 0)
        await show_env_menu(update, ctx, bid, last_page)
    elif data=="help":
        ...
        await show_home_card(update, ctx); return
        ...
        await safe_edit(q, "⚠️ <b>Delete this bot?</b>",
                        kb_confirm_delete(bid), parse_mode="HTML")
        ...
        txt=(f"⚙️ <b>Settings</b>\n\n"
             ...
             f"Premium: <code>{'yes' if await DB.is_premium(u.id) else 'no'}</code>\n"
             f"Bots: <code>{await DB.count_user_bots(u.id)}</code>")
        await safe_edit(q, txt, kb_back("menu"), parse_mode="HTML")
...
async def show_my_bots(update, ctx):
    ...
    for bid,info in bots.items():
        ...
        try:
            bot_lim=await ensure_bot_limits(bid)
            runtime_memory=f"{typ} · {bot_lim['memory_limit_mb']} MB"
        except Exception:
            runtime_memory=typ
        table_rows.append(["🟢" if st==UI.RUNNING else "🔴", info["name"], runtime_memory])
    ...
    blocks=[
        rm.heading("🤖 My Bots"),
        rm.compact_table(["Status", "Name", "Runtime · Memory"], table_rows),
        *rich_button_rows,
        rm.button_row([_rich_callback_button(f"{UI.BACK} Back", "menu", "primary")]),
    ]
    msg = q.message
...
async def show_logs(update, ctx, bid):
    ...
    blocks = [rm.heading(f"📜 Logs: {info['name']}")]
    if logs:
        blocks.append(rm.code(logs[-3000:], "text"))
        blocks.append(_nav_back_row(f"bot_detail:{bid}"))
    else:
        blocks.append(rm.paragraph("No logs yet. If the bot just started, wait 30 seconds and refresh."))
        blocks.append(_nav_refresh_row(f"logs:{bid}"))
        blocks.append(_nav_back_row(f"bot_detail:{bid}"))
    msg=q.message
    if msg is not None:
        try:
            ...
            log_err(f"RICH: show_logs rich edit failed {bid}",traceback.format_exc())
    await safe_edit(q,txt,kb,parse_mode="HTML")
...
async def do_delete(update, ctx, bid):
    ...
    try:
        if os.path.exists(info["dir"]):
            await asyncio.to_thread(shutil.rmtree, info["dir"], ignore_errors=True)
    except Exception: pass
...
async def show_vps(update, ctx):
    ...
    markup=InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"{UI.BACK} Back",callback_data="menu")]])
    blocks=[rm.heading("📊 VPS Status"),rm.compact_table(["Metric","Value"],[["CPU",f"{st['cpu']:.1f}%"],["RAM",f"{st['ram_used']:.2f}/{st['ram_total']:.2f} GB ({st['ram_pct']}%)"],["Disk",f"{st['disk_used']:.2f}/{st['disk_total']:.2f} GB ({st['disk_pct']}%)"],["Uptime",str(st['uptime'])]]),_nav_refresh_row("refresh_vps"),_nav_back_row("menu")]
    msg=q.message
    if msg is not None:
        try:
            await _rich_edit_with_fallback(msg.chat_id,msg.message_id,blocks,is_media=(getattr(msg,"video",None) is not None or getattr(msg,"photo",None) is not None))
            return
        except rm.RichMessageError:
            log_err("RICH: show_vps rich edit failed",traceback.format_exc())
    await safe_edit(q,txt,markup,parse_mode="HTML")
...
async def begin_github_input(update,ctx):
    ...
    msg_src = q.message
    if msg_src is None:
        log_err("RICH: begin_github_input missing q.message")
        try: await q.answer("Message expired, please retry", show_alert=True)
        except Exception: pass
        return
    prompt_text = (
    ...
    # FIX_H1: safe_edit deletes media messages and sends a replacement, so
    # msg_src.message_id can point at a deleted message.  Send the prompt
    # explicitly and store the returned message's id, mirroring the
    # begin_limit_input / begin_admin_setting_input pattern.
    prompt_msg = await msg_src.chat.send_message(
        prompt_text, parse_mode="HTML", reply_markup=kb_back("deploy"))
    try:
        await msg_src.delete()
    except Exception:
        pass
    ctx.user_data["awaiting_github_url"] = {
        "msg_id": prompt_msg.message_id, "chat_id": prompt_msg.chat_id,
        "expires_at": time.time() + GITHUB_TIMEOUT_SECONDS,
    }
    ctx.user_data["github_timeout_task"] = asyncio.create_task(
        _github_timeout_worker(ctx, prompt_msg.chat_id, prompt_msg.message_id))
...
async def handle_github_url_input(update,ctx):
    ...
    async def _edit(msg_id, chat_id, text, parse_mode="HTML", markup=None):
        try:
            await ctx.bot.edit_message_text(chat_id=chat_id, message_id=msg_id,
                                            text=text, parse_mode=parse_mode, reply_markup=markup)
        except Exception as e:
            log_err(f"github progress edit failed: {e}")
    if time.time()>state.get("expires_at",0):
        _github_state_clear(ctx); await _edit(msg_id,chat_id,f"{UI.ERROR} GitHub URL input timed out."); return True
    url=(update.message.text or "").strip()
    if not github_url_valid(url):
        await _edit(msg_id,chat_id,
    ...
    if uid not in ADMIN_IDS:
        ...
        if cnt>=MAX_BOTS_PER_USER:
            await _edit(msg_id,chat_id,f"{UI.ERROR} Max {MAX_BOTS_PER_USER} bots per user."); return True
    if shutil.which("git") is None:
        await _edit(msg_id,chat_id,f"{UI.ERROR} Git is not installed on this server. Contact admin."); return True
    await _edit(msg_id,chat_id,
    ...
    async with DEPLOY_SEMAPHORE:
        try:
            ...
            git_dir=staging/".git"
            if git_dir.exists(): await asyncio.to_thread(shutil.rmtree, git_dir, ignore_errors=True)
            ...
            if sr["verdict"]=="clear":
                final_dir=HOSTED_BOTS_DIR/bot_id; await asyncio.to_thread(shutil.move, str(staging), str(final_dir)); staging=None
                ...
                if started:
                    ...
                    await _edit(msg_id,chat_id,txt,markup=InlineKeyboardMarkup([
                    ...
                    await _edit(msg_id,chat_id,f"{UI.WARN} <b>Bot registered, but couldn't start.</b>\n{UI.DIV}\n"
                                           "Check logs and try Start again."+credit_footer(),markup=kb_back("my_bots"),parse_mode="HTML")
            else:
                ...
                await _edit(msg_id,chat_id,f"{UI.PENDING} <b>Under Review</b>\n{UI.DIV}\n"
            ...
            await _edit(msg_id,chat_id,f"<b>{err_action('GitHub clone timed out')}</b>\n{UI.DIV}\nTry again later.{credit_footer()}",
                                   parse_mode="HTML", markup=kb_back("deploy"))
        except (ValueError,RuntimeError) as e:
            log_err(f"github deploy {type(e).__name__}",traceback.format_exc())
            await _edit(msg_id,chat_id,f"<b>{err_action('GitHub deploy failed')}</b>\n{UI.DIV}\n{_html_text(str(e),300)}{credit_footer()}",
                                   parse_mode="HTML", markup=kb_back("deploy"))
        except Exception:
            log_err("github deploy unexpected",traceback.format_exc())
            await _edit(msg_id,chat_id,f"<b>{err_system('GitHub deploy failed')}</b>\n{UI.DIV}\nCouldn't complete the deployment.{credit_footer()}",
                                   parse_mode="HTML", markup=kb_back("deploy"))
        finally:
            if staging and staging.exists(): await asyncio.to_thread(shutil.rmtree, staging, ignore_errors=True)
    return True
...
async def execute_approval(bot_id, admin_id, approve: bool):
    ...
    try:
        ...
        await asyncio.to_thread(shutil.move, str(staging), str(final_dir))
...
# ================== ADMIN PRE-WARM ==================
async def cmd_prewarm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    u = update.effective_user
    if not u or u.id not in ADMIN_IDS:
        return
    status = await update.message.reply_text("⏳ Pre-warming common packages...")
    common = [
        "aiosqlite","pyTelegramBotAPI","python-telegram-bot","aiogram",
        "requests","aiohttp","python-dotenv","beautifulsoup4","lxml","Pillow","qrcode[pil]",
        "pyrogram","telethon","sqlalchemy","asyncpg","flask","fastapi","uvicorn",
        "fonttools","reportlab","pypdf","PyPDF2","fpdf2","python-docx",
        "pdf2image","pymupdf","pytesseract","numpy","pandas","openpyxl","gTTS","edge-tts",
    ]
    try:
        r = await asyncio.to_thread(
            subprocess.run,
            [sys.executable, "-m", "pip", "install",
             "--no-cache-dir", "--disable-pip-version-check"] + common,
            capture_output=True, text=True, timeout=600,
        )
        out = ((r.stdout or "") + (r.stderr or ""))[-2500:]
        await status.edit_text(
            f"{'✅' if r.returncode == 0 else '❌'} Pre-warm complete\n"
            f"```\n{out}\n```", parse_mode="Markdown")
    except Exception as e:
        await status.edit_text(f"❌ Pre-warm failed: {e}")
...
def main():
    ...
    app.add_handler(CommandHandler("prewarm", cmd_prewarm))
