...
def box_title(text: str) -> str:
    """Return the top border + title line."""
    return f"╭═━──────༺𓆩✧𓆪༻──────━═╮\n│  {text}"
...
def box_empty() -> str:
    """Return an empty line inside the box."""
    return "│"

def box_field(icon: str, label: str, value: str, position: str = "middle") -> str:
    """Return a field line. position: first|middle|last."""
    prefix = {"first": "┌", "middle": "├", "last": "└"}[position]
    label_s = str(label)
    if len(label_s) > 10:
        label_s = label_s[:9] + "…"
    return f"│  {prefix} {icon} {label_s:<10} : {value}"

def box_section(icon: str, name: str) -> str:
    """Return a section header line."""
    return f"│  {icon} {name}"

def render_box(title: str, sections: list) -> str:
    """
    sections = [
        ("🎮 PLAYER PROFILE", [("👤", "Name", "John"), ("🆔", "UID", "123")]),
        ("📊 METRICS",      [("💖", "Sent", "+100"), ("🏆", "Total", "1000")]),
    ]
    Returns the full box as a string (WITHOUT ``` wrappers).
    """
    lines = [box_title(title), box_empty()]
    for i, (sec_icon_name, fields) in enumerate(sections):
        lines.append(box_section(*sec_icon_name.split(" ", 1)))
        for j, (icon, label, value) in enumerate(fields):
            pos = "first" if j == 0 else ("last" if j == len(fields)-1 else "middle")
            lines.append(box_field(icon, label, str(value), pos))
        if i < len(sections) - 1:
...
def _box_text(text: str, limit: int = 3900) -> str:
    """Wrap box text for Telegram's message-size limit."""
    text = str(text or "")
    if len(text) > limit:
...
async def show_env_menu(update, ctx, bid, page=0):
    ...
    if not env_available():
        await safe_edit(q, _box_text(render_box("🔐 ENVIRONMENT", [
            ("❌ STATUS",[("⚠️","Status","Encryption unavailable")])
        ])), kb_back(f"bot_detail:{bid}"), parse_mode=None)
        return
    ...
    try:
        ...
        fields=[]
        if not chunk:
            fields.append(("📭","Vars","None"))
        for row in chunk:
            value = decrypt_env_value(row["encrypted_value"])
            shown = mask_env_value(value) if row["is_secret"] else value
            shown = str(shown).replace("\n"," ")[:120]
            fields.append(("🔑",str(row["env_key"])[:18],shown))
        fields.append(("📊","Page",f"{page+1}/{pages}"))
        fields.append(("📦","Total",f"{total}/{ENV_MAX_KEYS}"))
        box=render_box(f"🔐 ENVIRONMENT — {_short_button_name(info['name'], 21)}", [("🔐 VARS",fields)])
        buttons = []
        ...
        nav = []
        if page > 0: nav.append(InlineKeyboardButton("⬅️ Prev", callback_data=f"env_list:{bid}:{page-1}"))
        nav.append(InlineKeyboardButton(f"{page+1}/{pages}", callback_data=f"env_list:{bid}:{page}"))
        if page < pages - 1: nav.append(InlineKeyboardButton("➡️ Next", callback_data=f"env_list:{bid}:{page+1}"))
        buttons.append(nav)
        ...
        await safe_edit(q, _box_text(box), InlineKeyboardMarkup(buttons), parse_mode=None)
    except Exception as e:
        log_err(f"show_env_menu {bid}: {type(e).__name__}", traceback.format_exc())
        await safe_edit(q, _box_text(render_box("🔐 ENVIRONMENT", [("❌ ERROR",[("⚠️","Status","Unavailable")])])), kb_back(f"bot_detail:{bid}"), parse_mode=None)
...
async def show_limits_menu(update,ctx,bid):
    ...
    try:
        ...
        box=render_box(f"⚙️ RESOURCE LIMITS — {_short_button_name(info['name'], 22)}", [
            ("📏 LIMITS", [
        ...
        await safe_edit(q,txt,fallback_markup,parse_mode=None)
    except Exception as e:
        log_err(f"show_limits_menu {bid}: {type(e).__name__}",traceback.format_exc()); await safe_edit(q,_box_text(render_box("⚙️ RESOURCE LIMITS", [("❌ ERROR",[("⚠️","Status","Unavailable")])])),kb_back(f"bot_detail:{bid}"),parse_mode=None)
...
async def admin_users(update,ctx,page=0):
    q=update.callback_query
    async with aiosqlite.connect(DATABASE_PATH) as conn:
        cur=await conn.execute("SELECT DISTINCT user_id FROM bot_registry ORDER BY user_id")
    ...
    for uid in ids_page:
        ...
        try:
            chat=await ctx.bot.get_chat(uid); uname=f"@{chat.username}" if chat.username else "@unknown"
        except Exception: uname="@unknown"
        sections.append((f"👤 {uid}",[
            ("🆔","UID",uid),("👤","Username",uname),("🤖","Bots",bots),
            ("💎","Premium","yes" if prem else "no"),("🔇","Muted","yes" if muted else "no"),("🚫","Ban","yes" if banned else "no")
        ]))
    await safe_edit(q,_box_text(render_box(f"👥 USERS — {total} total",sections)),admin_page_markup("users",page,total,ids_page),parse_mode=None)
...
async def admin_user_detail(update,ctx,uid):
    ...
    await safe_edit(q,_box_text(box),kb,parse_mode=None)
...
async def admin_all_bots(update,ctx,page=0):
    ...
    async with aiosqlite.connect(DATABASE_PATH) as conn:
        pending_count = int((await (await conn.execute(
            "SELECT COUNT(*) FROM pending_deploys WHERE status='pending'"
        )).fetchone())[0])
    btn=[]
    ...
    if ids:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            conn.row_factory=aiosqlite.Row
            limits_rows=await (await conn.execute(f"SELECT * FROM bot_limits WHERE bot_id IN ({placeholders})",ids)).fetchall()
        limits_map={row["bot_id"]:dict(row) for row in limits_rows}
    sections=[]
    for r in chunk:
        lim=limits_map.get(r["bot_id"]) or await ensure_bot_limits(r["bot_id"])
        ...
        sections.append((f"🤖 {_short_button_name(r['name'], 24)}",[
            ("📛","Name",_short_button_name(r["name"],28)),
            ("👤","Owner",r["user_id"]),
            ("📡","Status",st),
            ("🧠","RAM",f"{lim['memory_limit_mb']} MB"),
        ]))
    ...
    box=render_box(f"🤖 ALL BOTS — {total} | pending: {pending_count}", sections)
    await safe_edit(q,_box_text(box),InlineKeyboardMarkup(btn),parse_mode=None)
...
async def admin_bot_detail(update,ctx,bid):
    ...
    async with aiosqlite.connect(DATABASE_PATH) as conn:
        conn.row_factory=aiosqlite.Row
        r=await (await conn.execute("SELECT * FROM bot_registry WHERE bot_id=?",(bid,))).fetchone()
    ...
    kb=InlineKeyboardMarkup([
        [InlineKeyboardButton("📊 Details",callback_data=f"bot_detail:{bid}"),InlineKeyboardButton("⚙️ Limits",callback_data=f"limits_menu:{bid}")],
        [InlineKeyboardButton("🛑 Force Stop",callback_data=f"admin_force_stop:{bid}"),InlineKeyboardButton("🗑️ Force Delete",callback_data=f"admin_force_del_confirm:{bid}")],
        [InlineKeyboardButton("📋 View Logs",callback_data=f"logs:{bid}")],
        [InlineKeyboardButton("🔙 Bots",callback_data="admin_bots:0")]])
    box=render_box("🤖 BOT DETAILS",[
        ("🤖 BOT INFO",[("📛","Name",_short_button_name(r["name"],40)),("🆔","Bot ID",_short_button_name(bid,40)),("⚙️","Type",r["type"]),("👤","Owner",r["user_id"])]),
        ("📡 STATUS",[("🟢" if st=="Running" else "🔴","Running",st),("🧠","Memory",f"{lim['memory_limit_mb']} MB"),("⚡","CPU",f"{lim['cpu_limit_percent']}%")]),
        ("🔧 ADMIN META",[("🔧","Service",_short_button_name(b.service_name,40)),("📅","Created",str(r["created_at"])[:19])])
    ])
    await safe_edit(q,_box_text(box),kb,parse_mode=None)
...
async def admin_server_stats(update,ctx):
    ...
    if not st:
        await safe_edit(q,_box_text(render_box("📊 SERVER STATS",[("❌ SYSTEM",[("⚠️","Status","psutil required")])])),kb_back("admin_panel"),parse_mode=None); return
    async with aiosqlite.connect(DATABASE_PATH) as conn:
        allocated=int((await (await conn.execute("SELECT COALESCE(SUM(memory_limit_mb),0) FROM bot_limits")).fetchone())[0])
    ...
    box=render_box("📊 SERVER STATS", [
        ("🖥 SYSTEM", [
            ("💻","CPU",f"{st['cpu']:.1f}%"),
            ("🧠","RAM",f"{st['ram_used']:.1f}/{st['ram_total']:.1f} GB ({st['ram_pct']}%)"),
            ("💾","Disk",f"{st['disk_used']:.1f}/{st['disk_total']:.1f} GB ({st['disk_pct']}%)"),
            ("🕒","Uptime",st["uptime"]),
        ]),
        ("⚙️ PM2", [
            ("📦","Total",pm2_count),("🟢","Active",active)
        ]),
        ("🗄️ DATABASE", [
            ("💾","Size",f"{dbsize:.2f} MB"),
            ("▶️","Panel Started",str(panel_started)[:19]),
            ("🛡️","Approval Started",str(approval_started)[:19]),
        ]),
    ])
    ...
    blocks=[
        ...
        rm.compact_table(["🖥️ SYSTEM", "Value"], [
            ["CPU", f"{st['cpu']:.1f}%"],["RAM Used", f"{st['ram_used']:.1f}/{st['ram_total']:.1f} GB"],
            ["RAM %", f"{st['ram_pct']}%"],["Disk Used", f"{st['disk_used']:.1f}/{st['disk_total']:.1f} GB"],["Uptime", st["uptime"]],
        ]),
        rm.compact_table(["⚙️ PM2", "Value"], [["Total Processes",str(pm2_count)],["Active Bots",str(active)],["Registered Bots",str(totalbots)],["Allocated RAM",f"{allocated} MB"]]),
        rm.compact_table(["🗄️ DATABASE", "Value"], [["DB Size",f"{dbsize:.2f} MB"],["Panel Started",str(panel_started)[:19]],["Approval Started",str(approval_started)[:19]]]),
        rm.button_row([_rich_callback_button("🔄 Refresh","admin_stats","primary"),_rich_callback_button("🔙 Admin","admin_panel","primary")]),
    ]
    msg=q.message
    if msg is None:
        log_err("RICH: admin_server_stats missing q.message"); return
    try:
    ...
    await safe_edit(q,_box_text(box),fallback_markup,parse_mode=None)
...
async def admin_audit_view(update,ctx,page=0,filter_kind="all",filter_value=None):
    ...
    async with aiosqlite.connect(DATABASE_PATH) as conn:
        conn.row_factory=aiosqlite.Row
        rows=await (await conn.execute(f"SELECT * FROM admin_audit {where} ORDER BY id DESC LIMIT ? OFFSET ?",(*args,AUDIT_PAGE_MAX,page*AUDIT_PAGE_SIZE))).fetchall()
    ...
    for r in rows[:AUDIT_PAGE_SIZE]:
        fields.append(("📋","Time",str(r["created_at"])[:19]))
        fields.append(("🔧","Admin",r["admin_id"]))
        fields.append(("⚡","Action",str(r["action"])[:35]))
        fields.append(("🎯","Target",str(r["target"] or "-")[:35]))
    if not fields: fields=[("📭","Entries","None")]
    kb=InlineKeyboardMarkup([
        [InlineKeyboardButton("All",callback_data="admin_audit:0:all"),InlineKeyboardButton("By Admin",callback_data="admin_audit_filter_admin"),InlineKeyboardButton("By Action",callback_data="admin_audit_filter_action")],
    ...
    await safe_edit(q,_box_text(render_box(f"📋 AUDIT LOG — {total} entries",[("📋 LIST",fields)])),kb,parse_mode=None)
...
async def admin_limits_defaults(update,ctx):
    ...
    total,cap,maxbot=get_vps_memory_cap_mb()
    box=render_box("⚙️ RESOURCE DEFAULTS",[
        ("⚙️ DEFAULTS",[("🧠","Default Mem",f"{d} MB"),("📈","Max Memory",f"{m} MB (runtime {maxbot} MB)"),("🧱","VPS Cap",f"{c}%")])
    ])
...
async def admin_maintenance(update,ctx):
    ...
    box=render_box("🔧 MAINTENANCE",[
        ("📡 STATE",[("🔧","Maintenance","ON" if mm=="1" else "OFF"),("📦","Deploys","ON" if de=="1" else "OFF"),("🔒","Panel Lock","UNLOCKED" if unlocked else "LOCKED")])
    ])
...
async def safe_edit(q, text, markup=None, parse_mode="HTML"):
    # FIX_PM_BOX_1: Code-box screens must use Markdown, never HTML.
...
async def _rich_edit_with_fallback(chat_id: int, message_id: int, blocks: list, token=None, is_media: bool = False) -> dict:
    ...
    return await client.replace(chat_id, message_id, blocks)
...
async def show_help(update, ctx):
    q=update.callback_query
    box = render_box("ℹ️ HOW IT WORKS", [
        ("🧭 STEPS", [
            ("1️⃣", "Step", "Deploy a bot"),
            ("2️⃣", "Step", "Set environment vars"),
            ("3️⃣", "Step", "Set memory/CPU"),
            ("4️⃣", "Step", "Monitor logs/status"),
        ]),
        ("⌨️ COMMANDS", [
            ("▶️", "/start", "Main menu"),
            ("🔧", "/admin", "Admin panel"),
    ...
    footer=credit_footer().strip()
    if footer:
        box=box[:-len(box_bottom())-1] + f"\n│  {footer}\n" + box_bottom()
    txt=_box_text(box)
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    box=render_box("💠 VPS HOSTING MANAGER", [
        ("👋 WELCOME", [
            ("👤", "Name", u.first_name or u.full_name),
            ("🆔", "User ID", u.id),
        ]),
        ("🤖 YOUR BOTS", [
            ("📊", "Total", total_bots),
    ...
    # Fallback: try the welcome video first (previous behavior), then text.
    try:
    ...
    await update.message.reply_text(fallback_text, parse_mode="Markdown", reply_markup=kb_main())
...
async def cb_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action=="admin_user_bots" and admin_only(u.id) and len(parts)>=3:
        uid=int(bid); page=max(0,int(parts[2]))
        bots=await DB.get_user_bots(uid); items=list(bots.items())
        chunk=items[page*10:(page+1)*10]
        sections=[]
        rows=[]
        for k,v in chunk:
            sections.append((f"🤖 {_short_button_name(v['name'],24)}", [
                ("📛","Name",_short_button_name(v["name"], 40)),
                ("🆔","Bot ID",_short_button_name(k, 40)),
            ]))
            rows.append([InlineKeyboardButton(v["name"][:30],callback_data=f"admin_bot:{k}")])
        if not sections:
            sections=[("📭 BOTS",[("📦","Count","None")])]
        nav=[]
        ...
        box=render_box(f"🤖 BOTS OF USER — {uid}", sections)
        await safe_edit(q,_box_text(box),InlineKeyboardMarkup(rows),parse_mode=None)
        return
...
# Feature 1 callbacks remain unchanged below.
    if action=="env_menu" and bid: await show_env_menu(update,ctx,bid,0)
        ...
        box=render_box("⚠️ DELETE BOT", [("🗑️ CONFIRM", [("🤖","Bot ID",bid), ("⚠️","Action","Delete this bot")])])
        await safe_edit(q,_box_text(box),kb_confirm_delete(bid),parse_mode=None)
        ...
        box = render_box("⚙️ SETTINGS", [
            ("👤 USER", [
                ("🆔", "User ID", u.id),
                ("💎", "Premium", "yes" if await DB.is_premium(u.id) else "no"),
                ("🤖", "Bots",    await DB.count_user_bots(u.id)),
            ]),
        ])
        await safe_edit(q, _box_text(box), kb_back("menu"), parse_mode=None)
...
async def show_my_bots(update, ctx):
    ...
    if not bots:
        box=render_box("🤖 MY BOTS", [
            ("📦 STATUS", [("📭", "Bots", "None deployed")]),
            ("🚀 NEXT STEP", [("➕", "Action", "Deploy your first bot")]),
        ])
        txt=_box_text(box)
        kb=InlineKeyboardMarkup([
        ...
        await safe_edit(q,txt,kb,parse_mode=None); return
    ...
    for bid,info in bots.items():
        ...
        table_rows.append(["🟢" if running else "🔴", info["name"], typ])
    ...
    await safe_edit(q,txt,InlineKeyboardMarkup(rows),parse_mode=None)
...
async def show_bot_detail(update, ctx, bid):
    ...
    box=render_box("🤖 BOT DETAILS", [
        ("🤖 BOT INFO", [
            ("📛","Name",_short_button_name(info["name"], 40)),
            ("🆔","Bot ID",_short_button_name(bid, 40)),
            ("⚙️","Type",typ_name),
            ("👤","Owner",info["user_id"]),
        ]),
        ("📡 STATUS", [
            ("🟢" if running else "🔴","Running","Running" if running else "Stopped"),
            ("🧠","Memory",f"{lim['memory_limit_mb']} MB"),
            ("⚡","CPU",f"{lim['cpu_limit_percent']}%"),
            ("🔐","Env Vars",env_count),
        ]),
        ("⚙️ SYSTEM", [
            ("🔧","Service",_short_button_name(b.service_name, 40)),
            ("📅","Created",str(info.get("created_at","unknown"))[:19]),
        ]),
    ])
    txt=_box_text(box)
...
async def show_logs(update, ctx, bid):
    ...
    if not logs:
        ...
        content="No logs yet. If it just started, wait 30 seconds and refresh."
    else:
        content=str(logs)[-2600:]
    box=render_box(f"📋 LOGS — {_short_button_name(info['name'], 28)}", [
        ("🧾 META", [
            ("🆔","Bot ID",_short_button_name(bid, 40)),
            ("📏","Lines Shown",25 if logs else 0),
        ]),
        ("📜 LOG CONTENT", [
            ("📄","Raw",content.replace("\n"," ⏎ ")[:1800]),
        ]),
...
async def show_bot_status(update, ctx, bid):
    ...
    b = HostedBot(bid, info["name"], info["dir"], info["type"], info["user_id"])
    running = await asyncio.to_thread(b.is_running)
    box=render_box("📊 BOT STATUS", [
        ("🤖 BOT", [
            ("📛","Name",_short_button_name(info["name"], 40)),
            ("🆔","ID",_short_button_name(bid, 40)),
        ]),
        ("📡 STATUS", [
            ("🟢" if running else "🔴","Running","Running" if running else "Stopped"),
            ("🔧","Service",_short_button_name(b.service_name, 40)),
    ...
    await safe_edit(q,_box_text(box),InlineKeyboardMarkup([[
        InlineKeyboardButton("🔙 Back", callback_data=f"bot_detail:{bid}")
    ]]),parse_mode=None)
...
async def show_vps(update, ctx):
    q=update.callback_query; st=get_system_stats()
    if not st:
        await safe_edit(q,_box_text(render_box("🖥 VPS STATUS", [
            ("❌ SYSTEM",[("⚠️","Status","psutil unavailable")])
        ])),kb_back("menu"),parse_mode=None); return
    # Keep the existing data source and callbacks; render box directly.
    try:
        pm2_result=await asyncio.to_thread(subprocess.run,["pm2","jlist"],capture_output=True,text=True,timeout=5)
        plist=json.loads(pm2_result.stdout) if pm2_result.returncode==0 and pm2_result.stdout else []
        hosted_count=sum(1 for p in plist if p.get("pm2_env",{}).get("name","").startswith("hosted-bot-"))
    except Exception:
        hosted_count=0
    box=render_box("🖥 VPS STATUS", [
        ("🖥 SYSTEM", [
            ("💻","CPU",f"{st['cpu']:.1f}%"),
            ("🧠","RAM",f"{st['ram_used']:.1f}/{st['ram_total']:.1f} GB ({st['ram_pct']}%)"),
            ("💾","Disk",f"{st['disk_used']:.1f}/{st['disk_total']:.1f} GB ({st['disk_pct']}%)"),
        ...
        ("⚙️ PROCESSES", [
            ("📦","PM2 Count",hosted_count),
            ("🤖","Hosted Bots",hosted_count),
        ]),
    ])
    await safe_edit(q,_box_text(box),InlineKeyboardMarkup([
        [InlineKeyboardButton(f"{UI.REFRESH} Refresh",callback_data="refresh_vps")],
        [InlineKeyboardButton(f"{UI.BACK} Back",callback_data="menu")]]),parse_mode=None)
