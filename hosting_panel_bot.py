...
async def begin_github_input(update,ctx):
    ...
    await safe_edit(q, prompt_text, kb_back("deploy"), parse_mode="HTML")
    ctx.user_data["awaiting_github_url"] = {
        "msg_id": msg_src.message_id, "chat_id": msg_src.chat_id,
        "expires_at": time.time() + GITHUB_TIMEOUT_SECONDS,
    }
    ctx.user_data["github_timeout_task"] = asyncio.create_task(
        _github_timeout_worker(ctx, msg_src.chat_id, msg_src.message_id))
