...
async def _rich_edit_with_fallback(chat_id: int, message_id: int, blocks: list, token=None, is_media: bool = False) -> dict:
    ...
    # FIX_RICH_27: Bot API 10.3 rich-message edit counterparts are unreliable.
    # Delete the old message and send a fresh rich card so the rich look persists.
    # is_media is retained for signature compatibility with existing call sites.
    try:
        await asyncio.to_thread(rm.delete_message, chat_id, message_id, token)
    except rm.RichMessageError:
        # Best-effort delete; if the old message is already gone, still send the new card.
        log_err("RICH: delete old message failed, sending fresh rich card", traceback.format_exc())
    return await asyncio.to_thread(rm.send_blocks, chat_id, blocks, token)
