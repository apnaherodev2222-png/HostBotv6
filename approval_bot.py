...
async def show_admin_home(update,ctx):
    ...
    box=render_box("🔧 ADMIN PANEL",[
        ("⚙️ ACTIONS",[("📢","Broadcast","Send message"),("📋","Audit Log","View entries")])
    ])
    await update.message.reply_text(_box_text(box),parse_mode="Markdown",reply_markup=InlineKeyboardMarkup([
...
async def show_audit(update,ctx,page=0):
    ...
    if not feature2_ready():
        await safe_edit(q,_box_text(render_box("📋 AUDIT LOG",[
            ("⚠️ STATUS",[("📡","Status","Migration pending")])
        ])),InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Admin",callback_data="adm_home")]]),parse_mode=None)
        return
    try:
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            conn.row_factory=aiosqlite.Row
            rows=await (await conn.execute("SELECT * FROM admin_audit ORDER BY id DESC LIMIT ? OFFSET ?",(AUDIT_PAGE_SIZE,page*AUDIT_PAGE_SIZE))).fetchall()
    except sqlite3.OperationalError:
        await safe_edit(q,_box_text(render_box("📋 AUDIT LOG",[
            ("❌ STATUS",[("⚠️","Status","Audit table missing")])
        ])),InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Admin",callback_data="adm_home")]]),parse_mode=None)
        return
    fields=[]
    for r in rows:
        fields.extend([
            ...
            ("🔧","Admin",r["admin_id"]),
            ("⚡","Action",str(r["action"])[:35]),
            ("🎯","Target",str(r["target"] or "-")[:35]),
        ])
    ...
    await safe_edit(q,_box_text(render_box("📋 AUDIT LOG",[("📋 ENTRIES",fields)])),InlineKeyboardMarkup(kb),parse_mode=None)
...
async def cmd_start(update, ctx):
    ...
    if u.id not in ADMIN_IDS:
        await update.message.reply_text(
            f"{UI.ERROR} Admin only.\nYour ID: <code>{u.id}</code>{credit_footer()}",
            parse_mode="HTML"
        )
        return
    fallback_text = _box_text(render_box("🛡 APPROVAL BOT",[
        ("📡 STATUS",[("🟢","Ready","YES")]),
        ("⌨️ COMMANDS",[("▶️","/start","Bot home"),("⏳","/pending","Pending deploys"),("🔧","/admin","Admin panel")]),
    ]))
    blocks = [
        rm.heading("🛡 Approval Bot"),
...
async def cmd_pending(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect(DATABASE_PATH) as conn:
        conn.row_factory=aiosqlite.Row
        rows=await (await conn.execute("SELECT * FROM pending_deploys WHERE status='pending' ORDER BY created_at DESC LIMIT 50")).fetchall()
    if not rows:
        await update.message.reply_text(_box_text(render_box("⏳ PENDING DEPLOYS",[("📭 PENDING",[("📦","Deploys","None")])])),parse_mode="Markdown")
        return
    for r in rows:
        box=render_box("⏳ PENDING DEPLOYS",[(
            "📦 DEPLOY",[
                ("👤","User",r["user_id"]),
                ("📛","Bot name",str(r["name"])[:35]),
                ("🆔","Bot ID",str(r["bot_id"])[:35]),
                ("⚙️","Type",str(r["bot_type"])[:20]),
            ])])
        await update.message.reply_text(_box_text(box),parse_mode="Markdown",reply_markup=InlineKeyboardMarkup([[
