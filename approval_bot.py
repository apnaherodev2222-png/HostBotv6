...
def _auto_install_python_deps(bot_dir: str) -> tuple[bool, str]:
    ...
    if not os.path.isfile(req): return True, "no requirements.txt"
    try:
        with open(req, "r", encoding="utf-8", errors="replace") as f:
            if not f.read().strip(): return True, "empty requirements.txt"
    except Exception as e:
        return False, f"read failed: {e}"
    try:
        r = subprocess.run([sys.executable, "-m", "pip", "install", "--no-cache-dir", "--disable-pip-version-check", "-r", req],
                           capture_output=True, text=True, timeout=300, cwd=bot_dir)
        return r.returncode == 0, ((r.stdout or "") + (r.stderr or ""))[-1500:]
    except subprocess.TimeoutExpired: return False, "pip install timed out after 300s"
    except Exception as e: return False, f"pip failed: {type(e).__name__}: {e}"

def _auto_install_node_deps(bot_dir: str) -> tuple[bool, str]:
    """Run npm install inside bot_dir."""
    pkg = os.path.join(bot_dir, "package.json")
    if not os.path.isfile(pkg): return True, "no package.json"
    try:
        r = subprocess.run(["npm", "install", "--omit=dev", "--no-audit", "--no-fund"],
                           capture_output=True, text=True, timeout=300, cwd=bot_dir)
        return r.returncode == 0, ((r.stdout or "") + (r.stderr or ""))[-1500:]
    except subprocess.TimeoutExpired: return False, "npm install timed out"
    except Exception as e: return False, f"npm failed: {type(e).__name__}: {e}"
...
async def show_admin_home(update,ctx):
    await update.message.reply_text(
        f"🛡️ <b>APPROVAL CENTER</b>\n{UI.DIV}\n"
        f"Approval bot supports Broadcast and Audit Log."
        f"{credit_footer()}",
        parse_mode="HTML",
...
def start_pm2(bot_id, bot_dir, bot_type):
    ...
    try:
        ...
        if bot_type in ("nodejs", "whatsapp"):
            _ok, _out = _auto_install_node_deps(bot_dir)
            if not _ok:
                log(f"auto-install node deps for {bot_id}: {_out[:400]}")
            target = None
            ...
            _ok, _out = _auto_install_python_deps(bot_dir)
            if not _ok:
                log(f"auto-install python deps for {bot_id}: {_out[:400]}")
            else:
                log(f"auto-install python deps OK for {bot_id}: {_out[:120]}")
            r = subprocess.run(
...
async def _rich_replace_or_send(chat_id: int, message_id: int, blocks: list, token=None) -> dict:
    """Rich edit -> on failure send new rich card + delete old. Never plain-text fallback."""
    if not chat_id or not message_id:
        log_err("RICH: _rich_replace_or_send missing ids")
        raise rm.RichMessageError("missing chat_id/message_id")
    client = rich_client if not token or token == APPROVAL_BOT_TOKEN else rm.RichClient(token=token)
    if client is None:
        raise rm.RichMessageError("client not configured")
    try:
        async with client._get_semaphore():
            response = await asyncio.to_thread(client._call_api, "editMessageText",
    ...
    try:
        async with client._get_semaphore():
            await asyncio.to_thread(client._call_api, "deleteMessage",
...
async def cb_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ok:
        ...
        icon = err_action("Failed to process: ")
    try:
